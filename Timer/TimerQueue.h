...
public:
    ...
    // 如果定时器是重复的，重新启动这个定时器，否则invalid
    void restart(Timestamp now); 

    static int64_t numCreated() { return s_numCreated_; }

private:
    //typedef std::function<void()> TimerCallback;现在定义在Callbacks.h中
...
private:
    ...
    //往序列中新插入定时器，如果是最新的，返回true
    bool insert(Timer *timer);

    EventLoop *loop_; //每个定时器都属于一个事件循环
    const int timerfd_;
    Channel timerfdChannel_; //负责这个timerfd的IO事件的Channel

    // 按照Timer的到期时间排序保存Timer指针
    TimerList timers_;
